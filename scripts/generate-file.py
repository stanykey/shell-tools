...
def get_file_size_in_bytes(size_string):
    ...
    if result is not None:
        ...
        ratio = 1 if result.group('multiplier') is None else multiplier[result.group('multiplier').lower()]
        return int(number * ratio)
    return 0


def generate_random_string(size=1024, chars=ascii_uppercase + digits):
    return ''.join(choice(chars) for _ in range(size))


def generate_file(path, size, line_length=1024):
    with open(path, 'w') as file:
        for i in range(size // line_length):
...
if __name__ == "__main__":
    ...
    file_path = Path(options.file_path).absolute()
    if not file_path:
